...
parser.add_argument('--dynamic-shape', type=str, default='mid', metavar='NAME',
                    help='dynamic-shape'),
############ 2.9加入，nj-TTA #######################
parser.add_argument('--nj-tta', type=int, default=0,
                    help='norm-jitter tta(default: 0)')
########################################################
...
#             _logger.info(norm_jitter)
    ...
    loader_train = create_loader(
        ...
        nj_dynamic=args.nj_dynamic, #2.17添加
...
#         logger = _logger
    ...
    loader_eval = create_loader(
        ...
        pin_memory=args.pin_mem,
        nj_tta=args.nj_tta, # 2.28添加，nj_tta
        norm_jitter=norm_jitter #12.20 添加，一个保存有所有元素的字典
#         logger = _logger
    )
...
#         logger = _logger
    ...
    try:
        ...
        Dynamic_P = Dynamic_P_class(epochs=args.epochs, batch_per_epoch=int(len(dataset_train)/args.batch_size)+1, dynamic_factor=args.dynamic_factor, shape=args.dynamic_shape) #2.9新加，传递函数为引用类型即可
        for epoch in range(start_epoch, args.epochs): #1.21修改，直接改成num_epochs->args.epochs，0-49
            ...
            eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, nj_tta=args.nj_tta)
            
            #12.20修改，增加test评估
            test_metrics = validate(model, loader_test, validate_loss_fn, args, nj_tta=args.nj_tta) 
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', nj_tta=0):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            if nj_tta == 0 :
                ...
                (input, target) = batch[0] #原始的test
                target = target.cuda()
                img_list = [input.cuda()]
                for idx in range(args.nj_tta):
                    img_list += [batch[idx+1][0].cuda()] #取第idx次增强的img
                out_mean = 0
                for idx in range(args.nj_tta+1):
                    out = model(img_list[idx])
                    out_mean = (out + out_mean * idx)/(idx+1)
                output = out_mean 
...
class Dynamic_P_class(object):
    ...
    def __init__(self, epochs=0, batch_per_epoch=0, dynamic_factor=1.0, shape=None):
        self.batches = epochs * batch_per_epoch #总batches数
        # 通过正弦*某值来调控到第几个epoch时是全力，*5时，3个epoch训练完后达到1，最后3个epoch恢复
        if shape=='mid': #两头低，中间高，用sin函数
            self.p_list = [math.sin(math.pi*idx / self.batches) * dynamic_factor for idx in range(self.batches)] #构造时就得到所有p的取值，sin的0-π
        elif shape=='right': #左边低，右边高，用sin函数的0-π/2，平均0.63
            self.p_list = [math.sin((math.pi/2)*idx / self.batches) * dynamic_factor for idx in range(self.batches)]
